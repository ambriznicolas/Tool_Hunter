import LowHigh from "../src/hooks/Sort.js";
let productList = [
  {
    product: "Twins Boxing Gloves Velcro MMA Muay thai Sparring Leather Green",
    image: "https://i.ebayimg.com/images/g/pPIAAOSwo2xnviPs/s-l500.webp",
    delivery: "Free Delivery",
    price: "$99.99",
  },
  {
    product: "Twins Boxing Gloves Velcro MMA Muay thai Sparring Leather White",
    image: "https://i.ebayimg.com/images/g/pPIAAOSwo2xnviPs/s-l500.webp",
    delivery: "152.99",
    price: "$159.99",
  },
  {
    product: "Twins Boxing Gloves Velcro MMA Muay thai Sparring Leather Red",
    image: "https://i.ebayimg.com/images/g/g8EAAOSwfKdn4JDI/s-l500.webp",
    delivery: "Free Delivery",
    price: "$111.99",
  },
  {
    product: "Twins Boxing Gloves Velcro MMA Muay thai Sparring Leather Maroon",
    image: "https://i.ebayimg.com/images/g/no8AAOSwAVNnR4-2/s-l500.webp",
    delivery: "5.99 Delivery",
    price: "$129.99",
  },
  {
    product: "Twins Boxing Gloves Velcro MMA Muay thai Sparring Leather Black",
    image: "https://i.ebayimg.com/images/g/cHEAAOSwvj9n~Xpe/s-l500.webp",
    delivery: "Free Delivery",
    price: "$95.99",
  },
];
console.log(LowHigh(productList));
export default productList;

// for (let i = 0; i < productList.length; i++) {
//   productList[i].price = productList[i].price.replace(/[^-.\d]/g, "");
// }
// productList.sort((p, o) => pap.price - o.price);
// console.log(productList);
